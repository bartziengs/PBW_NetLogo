...
to setup
  ...
  ;;simulate the house has been not been switched owner
  ask houses [
   set ticks-since-last-buy random (minimal-owner-living-period * 24)
  ]
...
to set-initial-capital ;;owners procedure
  ...
  [
    ;;make sure savings are not negative
...
to go
  ...
  let want-to-buy owners-with-house with [not (repayment = 0) and (income / 12) / repayment > moving-up-income-repayment] ;;have financial means to move up
  ...
  let want-to-sell owners-with-house with [not (repayment = 0) and repayment / (income / 12 ) < moving-down-income-repayment] ;;don't have financial means to live in this house, move down
...
;      set price-asked initial-sale-price + abs(initial-sale-price - random-normal initial-sale-price (initial-sale-price / (4 + random 10)))
  ...
  [
    ...
    ;;Because most bidding wars have a sequential conversion, adjust sale prices of buyer and seller towards each other
    let old-price price-asked
    let mean-offer mean ([price-offered] of turtle-set buyers)
    if (price-asked - mean-offer > 0)
    ;;apparently, the price asked is too high
    [
      ;;increase by a certain percentage
      set price-asked price-asked * ((101 + random 10) / 100)
    ]

    ;;calculate the shortcoming of the previous negotiation cycle and adjust with some random increase.
    ;;It can be expected that seller price is increasing so increase the bid not with the entire gap.
    ask turtle-set buyers[
      let gap old-price - price-offered
      set price-offered price-offered * ((101 + random 10 ) / 100)
    ]
  ]
...
;  print possible-purchase? self
  ...
  [
    ...
    ;;report a succesful negotiation and cancel this negotiation
